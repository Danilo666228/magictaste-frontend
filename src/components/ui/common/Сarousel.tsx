...
const Carousel = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & CarouselProps>(
	({ orientation = 'horizontal', opts, setApi, plugins, className, showControls = true, children, ...props }, ref) => {
	...
	({ className, variant = 'outline', size = 'icon', ...props }, ref) => {
		...
		return (
			<Button
				...
				size={size}
				className={cn(
					'absolute z-10 h-8 w-8 rounded-full opacity-70 transition-opacity hover:opacity-100',
					orientation === 'horizontal' ? 'left-2 top-1/2 -translate-y-1/2' : 'left-1/2 top-2 -translate-x-1/2 rotate-90',
					className
				)}
...
const CarouselDots = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => {
	const { api, currentIndex, scrollSnaps } = useCarousel()

	return (
		<div ref={ref} className={cn('mt-4 flex justify-center gap-1', className)} {...props}>
			{scrollSnaps.map((_, index) => (
				<button
					key={index}
					className={cn(
						'h-2 w-2 rounded-full transition-all',
						currentIndex === index ? 'w-4 bg-primary' : 'bg-muted-foreground/30 hover:bg-muted-foreground/50'
					)}
