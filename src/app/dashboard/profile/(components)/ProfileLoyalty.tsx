...
export function ProfileLoyalty() {
	...
	const points = loyalty?.data.points
	const pointsToNextLevel = loyalty?.data.pointsToNextLevel ?? 0
	const progress = points ? Math.min((points / (points + pointsToNextLevel)) * 100, 100) : 0
	return (
		<Card>
				...
				<div>
					<div className="flex items-center justify-between">
						<div className="flex flex-col gap-1">
							...
							<Typography className="text-sm text-muted-foreground">Зарабатывайте баллы и получайте
								преимущества</Typography>
						</div>
					...
					<div className="mb-8">
						...
						<Typography className="mt-2 text-sm text-muted-foreground">
							До следующего уровня осталось: <span
							className="font-medium text-primary">{pointsToNextLevel} баллов</span>
						</Typography>
					</div>
							...
							<ul className="space-y-3">
								{loyalty?.data.loyaltyLevel?.additionalBenefits?.map((benefit, index) => (
									<motion.li
										...
										{benefit.isActive ? <CircleCheckBig className="h-5 w-5 text-green-500" /> :
