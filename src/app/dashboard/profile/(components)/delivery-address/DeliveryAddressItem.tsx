...
export function DeliveryAddressItem({ deliveryAddress }: DeliveryAddressItemProps) {
	...
	const handleDelete = (e: React.MouseEvent) => {
		e.preventDefault()
		deleteDeliveryAddress({ config: { params: { addressId: deliveryAddress.id } } })
	}
			...
			<AccordionItem value={deliveryAddress.id} className='border-0'>
				<AccordionTrigger className='px-5 py-4 transition-colors'>
					<div className='flex w-full items-center justify-between gap-x-4'>
						...
						{/* <Button size='icon' onClick={handleDelete} variant={'outline'} className='group'>
							<Trash2 size={18} className='cursor-pointer text-muted-foreground group-hover:text-red-600' />
						</Button> */}
					</div>
				</AccordionTrigger>
