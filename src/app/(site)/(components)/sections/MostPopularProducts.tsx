	...
	return (
		<section className='relative py-20'>
			<div className='absolute inset-0 -z-10'>
				...
				<div className='bg-primary/3 absolute bottom-20 right-10 h-96 w-96 rounded-full blur-3xl' />
			</div>
			...
			<div className='container mx-auto'>
				...
				<div className='grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4'>
					{popularProducts.map((product, index) => (
						<Link href={ROUTE.product(product.id)} key={product.id} className='group relative'>
							<div className='relative h-80 overflow-hidden rounded-2xl bg-card shadow-lg transition-all duration-500 group-hover:-translate-y-1 group-hover:shadow-2xl group-hover:shadow-primary/10'>
								...
								<div className='bg-gradient-to-t from-card via-card/95 to-transparent p-5'>
									<Typography
										tag='h3'
				...
				<div className='mt-16 text-center'>
					<Link
						href={ROUTE.category()}
						className='group inline-flex items-center gap-2 rounded-full bg-primary px-8 py-4 font-semibold text-background shadow-lg transition-all duration-300 hover:bg-primary/90 hover:shadow-xl hover:shadow-primary/25'>
						Посмотреть всё меню
